...
class aeexp4(object):
	...
	def train(self):
		...
		if self.load_model_directory == None:
			...
			# --Build model--
			# Autoencoder
			H_attr = (tf.tanh(tf.matmul(X, W_attr)) + 1) / 2
			H_disp = (tf.tanh(tf.matmul(X, W_disp)) + 1) / 2
			X_recon = (tf.tanh( tf.matmul(H_attr, tf.transpose(W_attr)) + tf.matmul(H_disp, tf.transpose(W_disp)) ) + 1) / 2
