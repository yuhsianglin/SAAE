...
class aeexpsae(object):
	def __init__(self, input_dim, hid_dim, class_num, d1, lrn_rate, momentum, batch_size_train, epoch_max, reg_lambda, train_file_name, val_file_name, test_file_name, log_file_name_head, gaus_train_file_name, gaus_val_file_name, gaus_test_file_name, attr_file_name, write_model_log_period, lambda_coef):
		...
		self.attrdata = attrdataset.attrdataset(attr_file_name)
	...
	def eval_gen_loss(self, X, WTS, WX, S):
		# Note that tf.nn.l2_loss() has already included the 1/2 factor
		# loss = ave_entropy + self.reg_lambda * ( tf.nn.l2_loss(W_e) )
		gen_loss = tf.reduce_mean(tf.reduce_sum(tf.pow(X - WTS, 2), axis = 1)) + self.lambda_coef * tf.reduce_mean(tf.reduce_sum(tf.pow(WX - S, 2), axis = 1))
		return gen_loss
	...
	def train(self):
		...
		S = tf.placeholder( tf.float32, [None, self.hid_dim] )

		WTS = tf.matmul(tf.transpose(W_e), S)
		WX = tf.matmul(W_e, X)

		gen_loss = self.eval_gen_loss(X, WTS, WX, S)
		...
		while epoch < self.epoch_max:
			...
			while self.data.has_next_batch():
				...
				S_batch = self.attrdata.next_batch(Y_batch)
				feed_dict = { X: X_batch, S: S_batch }
		...
		s = tf.placeholder(tf.float32, [self.hid_dim])
		# Compute negative distance, in order to pick top k as shortest distances
		neg_dist_from_s = -tf.reduce_sum( tf.pow(WX - s, 2), axis = 1 )

		# Attribute row vectors of unsceen classes
		S_test_full = self.attrdata.X

		# 改到這裡

		neg_dist_matrix = []

		for s_vec in S_test_full:
			feed_dict = { X: X_test_full, s: s_vec }
