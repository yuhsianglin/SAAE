...
class aeexp1(object):
	...
	def train(self):
		...
		if self.load_model_directory == None:
			...
			X_recon = tf.matmul(H_attr, tf.tranpspose(W_attr)) + tf.matmul(H_disp, tf.transpose(W_disp))
		...
		for epoch in range(1, self.epoch_max + 1):
			...
			while self.data.has_next_batch():
				...
				_, train_match_loss_got, train_recon_loss_got = sess.run([train_gen_step, match_loss, recon_loss], feed_dict = feed_dict)
