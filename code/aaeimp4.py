...
class aaeimp4(object):
	...
	def train(self):
		...
		if self.load_model_directory == None:
			...
			rng_ae_2 = 1.0 / math.sqrt( float( self.hid_dim + self.hid_dim ) )
			W_e_2 = tf.Variable( tf.random_uniform( [self.hid_dim, self.hid_dim], minval = -rng_ae_2, maxval = rng_ae_2 ), name = "W_e_2" )
			b_e_2 = tf.Variable(tf.zeros([self.hid_dim]), name = "b_e_2")
			...
			H_2 = tf.sigmoid( tf.matmul(H, W_e_2) + b_e_2 )
			...
			# Implicit matching: use text feature/attribute T
			...
			match_loss = tf.reduce_mean( tf.reduce_sum( tf.pow(T - H_2, 2), axis = 1 ) )
			...
			# Training objectives
			train_gen_step = tf.train.AdagradOptimizer(self.lrn_rate).minimize(match_loss)
			...
			neg_dist_from_t = -tf.reduce_sum( tf.pow(H_2 - t, 2), axis = 1 )
