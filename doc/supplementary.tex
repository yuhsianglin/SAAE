...
\section{Adversarial autoencoder with the prior as the regularizer}
	...
	\end{align}
	where $\odot$ denotes the element-wise product, the log function is applied element-wisely, and $c_i$ is the class of the instance $x^{(i)}$. Here we assume that there is only one text $\bb{t}^c$ for each class $c$.
