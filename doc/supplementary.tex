...
\section{Adversarial autoencoder with the prior as the regularizer}
	...
	\end{align}
	is minimized. Here $\odot$ denotes the element-wise product, and the log function is applied element-wisely.
	...
	\begin{align}
		f_g = \frac{1}{m} \sum_{c = 1}^{N_s} \sum_{i = 1}^{m_c} \left\{ -\bb{x}^{c, (i)} \odot \log \tilde{\bb{x}}^{c, (i)} + \log\left[ 1 - D( \bb{h}^{c, (i)} ) \right] + || \bb{h}^{c, (i)} - \bb{t}^c ||_2^2 \right\},
	\end{align}
	where
	\begin{align}
		m = \sum_{c = 1}^{N_s} m_c
	...
	For the discriminator of the adversarial net, the loss function is the empirical risk,
	\begin{align}
		f_d = -\frac{1}{m} \sum_{j = 1}^{m} \log D( \bb{z}^{(j)} ) - \frac{1}{m} \sum_{c = 1}^{N_s} \sum_{i = 1}^{m_c} \log\left[ 1 - D( \bb{h}^{c, (i)} ) \right],
	\end{align}
	where $\bb{z}^{(j)}$ are $m$ samples drawn from the prior distribution $\mathcal{N}(0, \lambda^2)$.
